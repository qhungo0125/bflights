...
class TicketController {
    ...
    verifyTicketOwner = async (req, res, next) => {
        try {
            ...
            if (!ticket) {
                throw new Error("Ticket was not found or has already been deleted.")
            }
            if (!user._id.equals(ticket.userId)) {
                throw new Error("You do not have permission to perform operations on the ticket.")
            }
            req.ticket = ticket
            next()
        } catch (error) {
    ...
    post = async (req, res) => {
        try {
            ...
            const user = await this.getUser(req)
            const newTicket = new Ticket(null, flightId, classOfTicket, user._id)
    ...
    delete = async (req, res) => {
        try {
            ...
            const increaseEmptySeatResult = await flightStatisticModel.increaseEmptySeat(
                ticket.flightId, ticket.classOfTicket
            )
